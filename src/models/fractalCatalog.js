...
export const fractalCatalog = [
  ...
  },
  {
    ...
    intro: {
      beginner: {
        overview: '4つの単純な変換をランダムに繰り返すだけで、シダの葉そっくりな形が浮かび上がるフラクタル。',
        feature: '点をひとつずつ打っていくだけなのに、自然界の植物に近い精巧な形が現れるのが特徴です。',
        application: '自然物の形を数式で表現する例として、CG・教育・アート作品で広く紹介されています。',
        howTo: 'depthを増やすほど点の数が増え、シダの細部まで埋まっていく様子を観察できます。',
      },
      advanced: {
        overview: 'IFS（反復関数系）をカオスゲームで描画する確率的フラクタル。4つのアフィン変換を確率1/85/7/7%で適用する。',
        feature: '反復点数が増えるほどアトラクタへ収束し、自己アフィン構造（自己相似ではない）を持つ点集合が形成されます。',
        application: 'IFS理論、確率的フラクタル生成、自然物のモデリング（葉脈・植生）の代表例として用いられます。',
        howTo: 'depthを上げて点密度の変化を確認し、各変換の係数を変えると葉の形がどう変わるかを比較します。',
      },
    },
  },
]
