点$\mathbf{v}=(x,y,z)\in\mathbb{R}^3$を球面座標$(r,\theta,\phi)\ (r=|\mathbf{v}|,\theta=\operatorname{atan2}(y,x),\phi=\arcsin(\frac{z}{r}))$
...
ここで$\mathbf{c}\in\mathbb{R}^3$は初期点（定数）、$\mathbf{v}_0 = \mathbf{0}$です。点$\mathbf{c}$がマンデルバルブに属するのは、この軌跡列{$\mathbf{v}_k$}が有界　($|\mathbf{v}_k| \nrightarrow \infty$)
